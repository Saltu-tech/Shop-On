...
<?php 
	...
	if (isset($_POST['register'])) 
	{
		...
		$query=mysqli_query($dbconn,"SELECT * FROM `provider` WHERE mobile='$mobile' OR email='$email' ");
		...
		if(empty($fname) || empty($mobile) || empty($email)  || empty($password)) {    
			...
			//updating the table
			$query = "INSERT INTO `provider` (name, mobile, email, password) 
	...
	else if(isset($_POST['login']))
	{
		...
		$query1=mysqli_query($dbconn,"SELECT * FROM `provider` WHERE mobile='$mobile' ");
		
		$query2=mysqli_query($dbconn,"SELECT * FROM `provider` WHERE mobile='$mobile' AND password='$pass'");
        ...
        //$id=$res['cust_id'];
			...
			$_SESSION['provider_id'] = $res['provider_id'];
...
<html lang="en">
	<head>
		<title>Shop provider | Registration | Login</title>
		<meta charset="utf-8">
		...
		<section class="section-content p-4">  
			<div class="container " style="width:400px;">
				...
				<div id="tab" class="btn-group" data-toggle="buttons-radio">
					<a data-toggle="tab" class="btn btn-success active" href="#login">Login</a> <p class="small">Already a provider ? </p>
					<a data-toggle="tab" class="btn btn-success" href="#signup">Signup</a><p class="small">New Product/Service Provider? </p>
				</div>
